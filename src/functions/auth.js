...
export const handleSignup = async ({ email, password }, { authDispatch, setIsLoading }, { setEmail, setPassword, setConfirmPassword }, { setIsEmailError, setIsPassError }, { setIsEmailErrorMsg, setPassErrorMsg },
    navigation) => {
    try {
        setIsLoading(true)
...
export const handleLogin = async (
    email, password, { setEmail, setPassword }, setIsLoading, authDispatch, { setEmailError, setPasswordError, setLoginError }, navigation) => {
    try {
        setIsLoading(true)
...
export const confirmCode = async (code, token,
    ...
    try {
        ...
        if (error.code === "auth/invalid-verification-code") {
            ...
            setCodeError("This phone number is already associated with a different user account.")
...
export const forgotPassword = async (email, setisLoading, sendEmailSuccess) => {
    try {
        console.log("email", email);
        setisLoading(true)
        // await sendPasswordResetEmail(webAuth, email)
        await auth().sendPasswordResetEmail(email)
        sendEmailSuccess("An email has been sent to this email address")
        setisLoading(false)
    } catch (error) {
        console.log("forgote pass error", error);
        sendEmailSuccess("Can't send email")
        setisLoading(false)
    }
}
